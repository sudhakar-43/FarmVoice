...
class FarmVoiceAgent:
    ...
    async def process_message(
        self, message: str, user_id: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                "lat" in full_context
                and "lon" in full_context
                and full_context["lat"]
                and full_context["lon"]
            ):
                if not full_context.get("location") and not full_context.get("city"):
                    try:
                        ...
                        logger.info(
                            f"Auto-resolving location for {full_context['lat']}, {full_context['lon']}"
                        )
                        geo_info = await weather_service.reverse_geocode(
                            float(full_context["lat"]), float(full_context["lon"])
                        )
                        if geo_info:
                            full_context.update(
                                {
                            ...
                            logger.info(
                                f"Resolved location context: {geo_info.get('location_name')}"
                            )
            ...
            if detected_intent in REQUIRED_PRECONDITIONS:
                ...
                if required_field == "location":
                    ...
                    has_requirement = bool(
                        full_context.get("location")
                        or full_context.get("city")
                        or full_context.get("state")
                        or (full_context.get("lat") and full_context.get("lon"))
                        or profile.get("location")
                        or profile.get("location_address")
                ...
                # elif required_field == "soil_type": ...
                    ...
                    has_requirement = bool(
                        full_context.get(required_field)
                        or full_context.get("user_profile", {}).get(required_field)
                    )

                if not has_requirement:
                    logger.warning(
                        f"SAFETY GATE: Blocked {detected_intent} due to missing {required_field}"
                    )
                    agent_decision = {
            ...
            if (
                current_speech
                and message
                and current_speech.lower() == message.lower().strip()
            ):
                logger.warning("ANTI-ECHO: Agent echoed user input. Forcing fallback.")
                agent_decision["speech"] = (
                    "I'm listening. Could you give me more details?"
                )
                ...
                for action_spec in agent_decision["actions"]:
                    try:
                        ...
                        if result.get("success", True) and "result" in result:
                            tool_results[f"{action.type}_{action.entity}"] = result[
                                "result"
                            ]
            ...
            if tool_results:
                try:
                    # Build more specific synthesis prompt
                    synthesis_context = {
                    ...
                    if (
                        "recommend_crops" in tool_results
                        or agent_decision.get("intent") == "crop_recommendation"
                    ):
                        synthesis_instruction = "Summarize the crop recommendations in one sentence with 2-3 specific crop names."
                    elif (
                        "diagnose_disease" in tool_results
                        or agent_decision.get("intent") == "disease"
                    ):
                        synthesis_instruction = "Summarize the disease diagnosis with treatment options in one sentence."
                    elif "read_weather" in tool_results:
                        synthesis_instruction = "Summarize current weather and provide one actionable farming tip."
                    elif "read_market" in tool_results:
                        synthesis_instruction = (
                            "Summarize the market prices for 2-3 crops in one sentence."
                        )

                    user_query_for_synthesis = f"{synthesis_instruction}\nTool Results: {json.dumps(tool_results)}"

                    synthesis = await llm_service.generate_response(
                        role="synthesizer",
                        context=synthesis_context,
                        user_query=user_query_for_synthesis,
                    )

                    if synthesis and synthesis.get("speech"):
                        new_speech = synthesis["speech"].strip()
                        # Only use new speech if it's substantially different and better
                        if len(new_speech) > 0 and new_speech.lower() != speech.lower():
                            speech = new_speech
                            logger.info(
                                f"Synthesized new speech from tool results: {speech}"
                            )
            ...
            logger.info(
                f"Response generated: {len(actions_taken)} actions, {len(suggestions)} suggestions"
            )
    ...
    async def _get_agent_decision(
        ...
        try:
            ...
            decision = await llm_service.generate_response(
                role="agent", context=agent_context, user_query=message
            )
    ...
    async def answer_query(self, query: str, context: Dict[str, Any]) -> str:
        ...
        try:
            response = await llm_service.generate_response(
                role="query_answerer", context=context, user_query=query
            )
            return response.get(
                "answer", response.get("speech", "I'm not sure about that.")
            )
        except Exception as e:
            logger.error(f"Query answering failed: {e}")
            return "I'm having trouble answering that right now."

    async def generate_suggestions(self, context: Dict[str, Any]) -> List[Suggestion]:
        """
    ...
    async def execute_action(
        self, action: AgentAction, user_id: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(self.tools, tool_name):
                ...
                return {"success": False, "error": f"Action {tool_name} not supported"}
    ...
    def _get_ui_update_for_action(
        self, action: AgentAction, result: Any
    ) -> Dict[str, Any]:
        """
