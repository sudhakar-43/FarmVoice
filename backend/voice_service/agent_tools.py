...
class AgentToolRegistry:
    ...
    async def read_profile(self, user_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            result = (
                self.supabase.table("farmer_profiles")
                .select("*")
                .eq("user_id", user_id)
                .execute()
            )
    ...
    async def update_profile(
        self, user_id: str, updates: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """Update user profile"""
        try:
    ...
    async def read_crop(
        self, user_id: str, crop_id: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Get selected crops for user"""
        try:
            query = (
                self.supabase.table("selected_crops").select("*").eq("user_id", user_id)
            )
            if crop_id:
    ...
    async def create_crop(
        self, user_id: str, crop_name: str, **kwargs
    ) -> Dict[str, Any]:
    ...
    async def update_crop(
        self, user_id: str, crop_id: str, updates: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """Update existing crop"""
        try:
            updates["updated_at"] = datetime.now().isoformat()
            result = (
                self.supabase.table("selected_crops")
                .update(updates)
                .eq("id", crop_id)
                .eq("user_id", user_id)
                .execute()
    ...
    async def read_task(
        ...
        **kwargs,
        ...
        try:
            query = (
                self.supabase.table("daily_tasks").select("*").eq("user_id", user_id)
            )
            if task_id:
    ...
    async def create_task(
        self, user_id: str, task_name: str, scheduled_date: str, **kwargs
    ) -> Dict[str, Any]:
        """Create a new task"""
        try:
            task_data = {
    ...
    async def update_task(
        self, user_id: str, task_id: str, updates: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """Update an existing task"""
        try:
            result = (
                self.supabase.table("daily_tasks")
                .update(updates)
                .eq("id", task_id)
                .eq("user_id", user_id)
                .execute()
    ...
    async def read_notification(
        self, user_id: str, notification_id: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Get notifications for user"""
        try:
            query = (
                self.supabase.table("notifications").select("*").eq("user_id", user_id)
            )
            if notification_id:
    ...
    async def create_notification(
        self, user_id: str, title: str, message: str, **kwargs
    ) -> Dict[str, Any]:
    ...
    async def update_notification(
        self, user_id: str, notification_id: str, updates: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
    ...
    async def delete_notification(
        self, user_id: str, notification_id: str, **kwargs
    ) -> Dict[str, Any]:
    ...
    async def read_market(
        ...
        **kwargs,
        ...
        """Get market prices"""
        try:
            prices = await market_service.get_prices(
    ...
    async def diagnose_disease(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                word in symptoms_lower for word in ["brown", "spot", "blight", "leaf"]
            ):
                diagnosis = {
                    "name": "Leaf Blight",
                    "severity": "Moderate",
    ...
    async def read_health(
        ...
        try:
            query = (
                self.supabase.table("crop_health").select("*").eq("user_id", user_id)
            )
            if crop_id:
    ...
    async def create_health(
        self, user_id: str, crop_id: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "health_record": result.data[0] if result.data else None,
            }
        except Exception as e:
    ...
    async def read_soil(
        self, lat: float = None, lon: float = None, district: str = None, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    async def recommend_crops(
        ...
        **kwargs,
        ...
        try:
            ...
            if target_location and (lat is None or lon is None):
                logger.info(
                    f"Auto-resolving location for crop recommendation: {target_location}"
                )
                resolved = await web_scraper.get_location_from_name(target_location)
                if resolved:
                    lat = resolved.get("latitude")
                    ...
                    location_data["soil_type"] = location_data.get(
                        "soil_type"
                    ) or resolved.get("soil_type")
                    location_data["climate"] = location_data.get(
                        "climate"
                    ) or resolved.get("climate")
                    location_data["state"] = (
                        resolved.get("state") or location_data["state"]
                    )
            ...
            recommendations = crop_recommender.recommend_crops(
                location_data=location_data, supabase_client=self.supabase, limit=5
            )
            ...
            return {
                ...
                "resolved_location": location_data.get("district")
