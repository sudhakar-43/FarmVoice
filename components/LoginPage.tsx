...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const validatePhone = (phone: string) => {
    // Relaxed validation: any 10 digits
  ...
  const handleIdentifierChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Login Mode: Hybrid (Name or Phone)
    // If starts with digit -> Treat as phone -> numeric only
    const firstCharIsDigit = /^\d$/.test(val[0]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isDigits = /^\d+$/.test(phoneNumber);
    if (isDigits && phoneNumber.length !== 10) {
        ...
        <div className="hidden md:flex flex-col items-center justify-center text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-xl text-emerald-700 dark:text-emerald-300 max-w-md mx-auto leading-relaxed">
              Empowering farmers with AI-driven insights for a sustainable and
              prosperous future.
            </p>
          </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`input-field dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 ${
                    /^\d+$/.test(phoneNumber)
                      ? "!pl-12" /* Adjusted padding for +91 */
                      : "!pl-10" /* Standard padding for icon */
                  }`}
